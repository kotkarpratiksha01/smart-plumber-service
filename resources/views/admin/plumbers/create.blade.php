...
<div class="p-6">
    <div class="max-w-3xl mx-auto bg-slate-800 p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-semibold mb-4 text-white">Create Plumber</h2>
        ...
        <form action="{{ route('dashboard.plumbers.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm text-slate-300">Name</label>
                <input name="name" value="{{ old('name') }}" class="mt-1 block w-full rounded bg-slate-900 border border-slate-700 text-white p-2" required />
            </div>
            <div>
                <label class="block text-sm text-slate-300">Phone</label>
                <input name="phone" value="{{ old('phone') }}" class="mt-1 block w-full rounded bg-slate-900 border border-slate-700 text-white p-2" />
            </div>
            <div>
                <label class="block text-sm text-slate-300">Location</label>
                <input name="location" value="{{ old('location') }}" class="mt-1 block w-full rounded bg-slate-900 border border-slate-700 text-white p-2" />
            </div>
            <div>
                <label class="block text-sm text-slate-300">Services (comma separated)</label>
                @php
                    $__services_val = old('services');
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="rating" value="{{ old('rating', 4.5) }}" class="mt-1 block w-full rounded bg-slate-900 border border-slate-700 text-white p-2" />
                </div>
                <div>
                    <label class="block text-sm text-slate-300">Experience (years)</label>
                    <input name="experience_years" value="{{ old('experience_years', 2) }}" class="mt-1 block w-full rounded bg-slate-900 border border-slate-700 text-white p-2" />
                </div>
            </div>

            <div class="flex items-center space-x-3">
                <label class="flex items-center text-sm text-slate-300">
                    <input type="checkbox" name="available" value="1" checked class="mr-2" /> Available
                </label>
            </div>
